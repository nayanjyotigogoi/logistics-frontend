...
export const createMasterAwbsColumns = (options: CreateColumnsOptions | AppRouterInstance): ColumnDef<unknown>[] => {
  const router = 'push' in options ? options : options.router;
  const onDelete = 'push' in options ? undefined : options.onDelete;

  return [
  {
    accessorKey: 'master_number',
    header: 'Master AWB Number',
    size: 150,
    cell: ({ row }) => {
      const masterNumber = row.getValue('master_number') as string;
      return (
  ...
  },
  {
    ...
    size: 120,
    cell: ({ row }) => {
      const masterAwb = row.original as any;
      ...
      return (
        <div className="flex items-center space-x-1">
          ...
          <button
            ...
            onClick={() => router.push(`/dashboard/master-awbs/${masterAwb.master_id}/edit`)}
            title="Edit"
          >
          ...
          <button
            ...
            onClick={() => {
              if (onDelete) {
                onDelete(masterAwb.master_id, masterAwb.master_number);
              } else if (confirm(`Are you sure you want to delete Master AWB ${masterAwb.master_number}?`)) {
                console.log('Delete master AWB:', masterAwb.master_id);
